...
%rho, theta, psi one by one, and form the whole time derivative using 
%the chain rule and summing the deriviatives)

dRba = (diff(Rba,rho))*drho+(diff(Rba,theta))*dtheta+(diff(Rba,psi))*dpsi;

% Use the formulat relating Rba, dRba and Omega 
%(skew-symmetric matrix underlying the angular velocity omega)
Omega = dRba*Rba';
...
% This line creates a Matlab function returning 
%Rba and M for a given A  = [rho;theta;psi], 
%can be called using [Rba,M] = Rotations(state);
